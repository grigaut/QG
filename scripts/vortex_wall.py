...
if freq_plot > 0:
    ...
    f2.suptitle(
        "Evolution of the potential vorticity q (units of $f_0$) and stream function contours"
    )
...
for n in range(1, n_steps + 1):
    if n in ns_plot:
        ...
        i, j = ind // 4, ind % 4
        im = a2[i, j].imshow(
            q_over_f0.T, cmap=palette, origin="lower", vmin=0, vmax=wM, animated=True
        )
        ...
        a2[i, j].contour(
            ...
            colors="grey",
            origin="lower",
            levels=np.linspace(psi.min() + eps, psi.max() - eps, 6),
        )
    ...
    if freq_plot > 0 and (n % freq_plot == 0 or n == n_steps):
        ...
        if wM is None or pM is None:
            ...
            pM = np.abs(psi).max()
        im0 = a[0].imshow(
            q_over_f0.T, cmap=palette, origin="lower", vmin=-wM, vmax=wM, animated=True
        )
        im1 = a[1].imshow(
            psi[0, 0].T, cmap="bwr", origin="lower", vmin=-pM, vmax=pM, animated=True
        )
        if n // freq_plot == 1:
