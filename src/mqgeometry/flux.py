...
def flux_3pts(q, u, dim):
    ...
    qi_left = torch.cat([qi_0, qi_left_in.narrow(dim, 0, n - 3), qi_m1], dim=dim)
    qi_right = torch.cat([qi_0, qi_right_in.narrow(dim, 1, n - 3), qi_m1], dim=dim)
...
def flux_5pts(q, u, dim):
    ...
    qi_left = torch.cat(
        [qi_0, qi_left_b.narrow(dim, 0, 1), qi_left_in, qi_left_b.narrow(dim, -1, 1)],
        dim=dim,
    )
    qi_right = torch.cat(
        [
